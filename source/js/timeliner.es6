...
$( () => {
  ...
  ,count = 0
  $.timeliner = (options) =>{
    if ($.timeliners == null || $.timeliners == undefined){
      $.timeliners = {
        options: []
      }
  ...
  var startTimeliner = (options)=>{
    ...
    var openStartEvents = (events) => {
      ...
      $.each(events, (index, value)=>{
        ...
        $(value).parents(settings.timelineTriggerContainer).show(settings.speed * settings.baseSpeed, () =>{
          openEvent($(value).find(settings.timelineTriggerAnchor), $(value + settings.EXContentIdSuffix))
        })
      })
    }
    var openEvent = (eventHeading, eventBody) => {
    ...
    var closeEvent = (eventHeading, eventBody) => {
      $(eventHeading).find('a')
      .animate({
